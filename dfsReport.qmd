...
@getSavingSessionTweets shows the cumulative number of #savingSession tweets collected and marks the relevant events. Labels are likely to overlap where events are close together.

```{r}
#| label: getSavingSessionTweets
...
```
@getDemandFlexibilityServiceTweets shows the cumulative number of #DemandFlexibilityService tweets collected and marks the relevant events. Labels are likely to overlap where events are close together.
...
```{r}
#| label: getDemandFlexibilityServiceTweets
...
Remember the size of the words in the word cloud is relative to the count of all negative words.
