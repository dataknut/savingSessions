...
if(dir.exists(dataPath)){
  ofile <- path.expand(paste0(dataPath, "tw_", searchString,"_", lubridate::now(),".csv"))
...
sessionDateStart <- lubridate::as_datetime("2022-11-15 17:00:00") # the half-hour it starts
sessionDateEnd <- lubridate::as_datetime("2022-11-15 18:00:00") # 

# plot over time

myCaption <- "Data: twitter API via @mkearney's rtweet package\nPlot by @dataknut\nSaving Session period marked as grey box"
ggplot2::ggplot(plotDT, aes(x = Time, y = nObs)) + 
                  geom_point() +
  annotate("rect", xmin = sessionDateStart,
           xmax = sessionDateEnd,
           ymin = ymin, ymax = ymax,
